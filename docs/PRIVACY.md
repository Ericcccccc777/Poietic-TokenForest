...
This has happened twice so far: the **price-table update** (v0.1.4, which added our website's hosting provider as a counterparty) and the **update check** (v0.2.1, which added GitHub). Both are off by default, download-only, and send no identifiers.
